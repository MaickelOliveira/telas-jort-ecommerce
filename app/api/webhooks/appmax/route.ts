...
export async function POST(request: NextRequest) {
  ...
  if (!startWebhook("appmax", eventId)) return NextResponse.json({ received: true, duplicate: true });
  after(async () => {
    try {
      if (payload.data.order_id) {
        const order = await fetchAppmaxOrder(String(payload.data.order_id));
        updateOrderPaymentByProviderId(String(order.id), String(order.status || "pending"), "appmax");
        const localOrder = getStoredOrderByProviderId(String(order.id));
        ...
        if (["order_refund", "order_partial_refund"].includes(payload.event)) {
          if (localOrder) {
            const pending = listRefundRequestsForOrder(localOrder.id).find((item) => item.status === "processing" && item.provider === "appmax");
            if (pending) completeAdminRefund({ requestId: pending.id, actor: "webhook:appmax" });
          }
        }
      }
      finishWebhook("appmax", eventId, "processed");
    } catch {
      finishWebhook("appmax", eventId, "failed");
    }
  });
