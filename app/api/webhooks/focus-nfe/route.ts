...
export async function POST(request: NextRequest) {
  ...
  if (!/^TJ-[A-Za-z0-9-]{3,80}$/.test(reference) || !await getFiscalDocumentByReference(reference)) return NextResponse.json({ error: "Referência desconhecida" }, { status: 404 });
  const eventId = `${reference}:${createHash("sha256").update(JSON.stringify(body)).digest("hex").slice(0, 24)}`;
  if (!await startWebhook("focus_nfe", eventId)) return NextResponse.json({ received: true, duplicate: true });
  try {
    const status = await syncFocusNfeWebhook(reference, { ...data, ...body } as Parameters<typeof syncFocusNfeWebhook>[1]);
    await finishWebhook("focus_nfe", eventId, "processed");
    return NextResponse.json({ received: true, status });
  } catch {
    await finishWebhook("focus_nfe", eventId, "failed");
